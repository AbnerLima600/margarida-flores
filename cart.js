      ...
      <div class="pm-section">
        ...
        <div class="tshirt-upload-zone" id="tshirtUploadZone" onclick="triggerTshirtUpload()" style="border:2px dashed #e91e8c;border-radius:10px;padding:20px;text-align:center;cursor:pointer;background:#fdf2f8;transition:background .2s;display:flex;flex-direction:column;align-items:center;justify-content:center">
          ...
          <div class="tshirt-upload-preview-wrap" id="tshirtUploadPreviewWrap" style="display:none;flex-direction:column;align-items:center;width:100%">
            ...
            <button type="button" class="tshirt-remove-photo" onclick="removeTshirtPhoto(event)" style="background:#f5f3ff;color:#ffd633;border:1px solid #fff0b8;padding:4px 10px;border-radius:20px;font-size:11px;font-weight:700;cursor:pointer">✕ Remover Foto</button>
          </div>
        </div>
      </div>
    `;
...
function openPremiumCardModal() {
  ...
  <div class="bottom-sheet card-modal-sheet">
    <div class="cm-header">
      <div class="cm-title">
        <svg viewBox="0 0 24 24" fill="none" stroke="#ffb300" stroke-width="2" width="18" height="18"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
    ...
    <div class="cm-footer">
      <button class="cm-save-btn" onclick="savePremiumCard()" style="background:#ffb300">
  ...
  <div id="checkoutScreen" class="checkout-screen">
    ...
    <div class="co-body">
          ...
          ${items.map(i => {
            ...
            if (i.is_alliance) {
              allianceSummaryDetails = `
                <div style="font-size:11px;color:#4b5563;margin-top:4px;background:#f5f3ff;border-left:2px solid #e91e8c;padding:4px 6px;border-radius:4px;font-family:'DM Sans',sans-serif;line-height:1.4">
      ...
      <div id="coStep1" class="co-card">
        ...
        <label class="co-lbl">Telefone</label>
      ...
      <div id="coStep2" style="display:none">
        ...
        <div class="co-card">
          ...
          <div id="cardFields" style="display:none;margin-top:12px">
            ...
            <input id="ccName" class="co-input" placeholder="NOME COMO ESTÁ NO CARTÃO" inputmode="text" autocomplete="cc-name" oninput="this.value=this.value.replace(/[0-9]/g,'').toUpperCase()">
            <div class="co-grid-2">
          ...
          <div class="co-paymethods">
            <span class="co-paymethods-lbl">Formas de pagamento aceitas</span>
            <div class="cc-brands cc-brands-inline">
              <div class="cc-brand cc-pix"><img src="https://logospng.org/download/pix/logo-pix-icone-512.png" width="22" height="22" alt="PIX"></div>
...
.comp-item.selected .comp-check{background:#ffc400;border-color:#ffc400}
...
.bq-card.selected .bq-img{border-color:#ffc400;box-shadow:0 0 0 2px #fff,0 0 0 4px rgba(255, 196, 0,.25)}
.bq-name{font-size:11px;font-weight:700;color:#374151;font-family:'DM Sans',sans-serif;text-align:center;line-height:1.1}
.bq-card.selected .bq-name{color:#ffc400}
...
.pm-add-btn{
  ...
  background:#ffc400;color:#3d2e00;border:none;
  font-weight:800;font-size:14px;cursor:pointer;
  display:flex;align-items:center;justify-content:center;gap:8px;
  transition:box-shadow .2s,transform .15s,background .2s;
  font-family:'Plus Jakarta Sans',sans-serif;
  box-shadow:0 4px 14px rgba(255, 196, 0,.28);
  letter-spacing:.2px;
}
.pm-add-btn:hover{background:#f0a800;box-shadow:0 6px 20px rgba(255, 196, 0,.42);transform:translateY(-1px)}
...
.pm2-paper.active{border-color:#ffb300!important;transform:scale(1.05)}
...
.us-continue-btn:hover{background:#f0a800;box-shadow:0 6px 20px rgba(255, 196, 0,.42);transform:translateY(-1px)}
...
.cart-clear-btn{background:none;border:none;color:#ffd633;font-size:12px;font-weight:600;cursor:pointer}
...
.cart-remove{background:#f5f3ff!important;border-color:#fff0b8!important;font-size:14px!important}
...
.cart-checkout-btn:hover{background:#f0a800;box-shadow:0 6px 20px rgba(255, 196, 0,.42)}
...
.co-back{
  ...
  color:#3d2e00;cursor:pointer;display:flex;
  width:34px;height:34px;border-radius:50%;
  align-items:center;justify-content:center;
  transition:background .2s;
}
.co-back:hover{background:rgba(0,0,0,.16)}
.co-title{
  font-size:16px;font-weight:800;color:#3d2e00;
  font-family:'Plus Jakarta Sans',sans-serif;
  letter-spacing:.2px;
}
...
.co-step.active .co-step-dot{background:#ffc400;color:#3d2e00;box-shadow:0 4px 12px rgba(255, 196, 0,.3)}
...
.co-step-line.active{background:#ffc400}
...
.co-input:focus{border-color:#ffc400;box-shadow:0 0 0 3px rgba(255, 196, 0,.08)}
...
.co-check-lbl input[type=checkbox]{accent-color:#ffc400}
...
.co-primary-btn:hover{background:#f0a800;box-shadow:0 6px 22px rgba(255, 196, 0,.45);transform:translateY(-1px)}
...
.co-field-error{color:#ffc400;font-size:12px;font-weight:600;margin:4px 18px 10px;font-family:'DM Sans',sans-serif;}
.co-grid-2 .co-field-error{margin:4px 2px 0;}
.co-input.co-input-error{border-color:#ffc400;}
...
.co-trocar{
  background:#f5f3ff;border:1px solid #fcd5d5;color:#ffc400;
...
.co-freight-opt.selected{border-color:#ffc400;background:#f5f3ff}
...
.co-radio-dot.selected{border-color:#ffc400;background:#ffc400}
...
.co-pay-opt.selected{border-color:#ffc400;background:#f5f3ff}
.co-pay-icon{
...
.cc-brand.cc-pix{padding:0;}
.cc-brand.cc-pix img{display:block;width:22px;height:22px;}
.co-paymethods{margin:14px 18px 2px;text-align:center;}
.co-paymethods-lbl{display:block;font-size:11px;color:#9aa0a6;font-weight:600;margin-bottom:8px;font-family:'DM Sans',sans-serif;letter-spacing:.2px;}
.co-paymethods .cc-brands-inline{margin:0;}
...
.co-pix-timer{display:inline-flex;align-items:center;gap:6px;justify-content:center;margin:0 auto 14px;padding:7px 15px;background:#fff5f5;border:1px solid #f3c2c5;border-radius:30px;color:#ffc400;font-size:13px;font-weight:700;font-family:'DM Sans',sans-serif}
...
#cartBadge{position:absolute;top:-6px;right:-6px;background:#ffd633;color:#3d2e00;font-size:11px;font-weight:700;width:18px;height:18px;border-radius:50%;display:none;align-items:center;justify-content:center;line-height:1}
...
.btn-cart {
  ...
  margin-bottom: 10px;
  letter-spacing: .2px;
  box-shadow: 0 4px 18px rgba(255, 196, 0,.28);
  transition: background .2s, box-shadow .2s, transform .15s;
}
.btn-cart:hover { background: #f0a800; box-shadow: 0 6px 24px rgba(255, 196, 0,.42); transform: translateY(-1px); }
...
.btn-buy {
  ...
  cursor: pointer;
  letter-spacing: .3px;
  box-shadow: 0 4px 18px rgba(255, 196, 0, .30);
  transition: box-shadow .2s, transform .15s, background .2s;
}
.btn-buy:hover { background: #f0a800; box-shadow: 0 6px 24px rgba(255, 196, 0, .45); transform: translateY(-1px); }
        ...
        <div class="pm-section" style="margin-top: 15px; text-align: left;">
          ...
          <div class="tshirt-upload-zone" id="tshirtUploadZoneDetail" onclick="triggerTshirtUploadDetailPage()" style="border:2px dashed #e91e8c;border-radius:10px;padding:20px;text-align:center;cursor:pointer;background:#fdf2f8;transition:background .2s;display:flex;flex-direction:column;align-items:center;justify-content:center">
            ...
            <div class="tshirt-upload-preview-wrap" id="tshirtUploadPreviewWrapDetail" style="display:none;flex-direction:column;align-items:center;width:100%">
              ...
              <button type="button" class="tshirt-remove-photo" onclick="removeTshirtPhotoDetailPage(event)" style="background:#f5f3ff;color:#ffd633;border:1px solid #fff0b8;padding:4px 10px;border-radius:20px;font-size:11px;font-weight:700;cursor:pointer">✕ Remover Foto</button>
            </div>
          </div>
        </div>
      `;
